...
@Component({
  selector: 'lib-chorus-pedal',
  standalone: true,
  imports: [CommonModule, KnobComponent],
  template: `
    @if (vm.vm$ | async; as state) {
      <div class="pedal" [class.disabled]="!state.pedals.chorus.enabled">
        <div class="jack-row">
          <div class="jack jack-in"></div>
          <div class="jack jack-out"></div>
        </div>
        <div class="pedal-header">
          <span class="pedal-name">CHORUS</span>
          <button
            class="power-btn"
            [class.active]="state.pedals.chorus.enabled"
            (click)="vm.togglePedal('chorus')"
          ></button>
        </div>
        <div class="controls" [class.bypassed]="!state.pedals.chorus.enabled">
          <lib-knob
            label="Rate"
            [minValue]="0" [maxValue]="100" [measureUnit]="'%'"
            [value]="state.pedals.chorus.rate * 100"
            (valueChange)="vm.updateChorus({ rate: $event / 100 })"
          />
          <lib-knob
            label="Depth"
            [minValue]="0" [maxValue]="100" [measureUnit]="'%'"
            [value]="state.pedals.chorus.depth * 100"
            (valueChange)="vm.updateChorus({ depth: $event / 100 })"
          />
          <lib-knob
            label="Mix"
            [minValue]="0" [maxValue]="100" [measureUnit]="'%'"
            [value]="state.pedals.chorus.mix * 100"
            (valueChange)="vm.updateChorus({ mix: $event / 100 })"
          />
        </div>
        <div class="pedal-color-strip" style="background: #3498db;"></div>
      </div>
    }
  `,
  styles: [`
...
export class ChorusPedalComponent {
