...
@Component({
  selector: 'lib-reverb-pedal',
  standalone: true,
  imports: [CommonModule, KnobComponent],
  template: `
    @if (vm.vm$ | async; as state) {
      <div class="pedal" [class.disabled]="!state.pedals.reverb.enabled">
        <div class="jack-row">
        ...
        <div class="pedal-header">
          ...
          <button
            class="power-btn"
            [class.active]="state.pedals.reverb.enabled"
            (click)="vm.togglePedal('reverb')"
          ></button>
        </div>
        <div class="controls" [class.bypassed]="!state.pedals.reverb.enabled">
          <lib-knob
            label="Mix"
            [minValue]="0" [maxValue]="100" [measureUnit]="'%'"
            [value]="state.pedals.reverb.mix * 100"
            (valueChange)="vm.updateReverb({ mix: $event / 100 })"
          />
          <lib-knob
            label="Decay"
            [minValue]="1" [maxValue]="50" [measureUnit]="'0.1s'"
            [value]="state.pedals.reverb.decay * 10"
            (valueChange)="vm.updateReverb({ decay: $event / 10 })"
          />
        </div>
        <div class="pedal-color-strip" style="background: #1abc9c;"></div>
      </div>
    }
  `,
  styles: [`
    .pedal { background: linear-gradient(180deg, #1a1a1a 0%, #2d2d2d 100%); border: 1px solid #555; border-radius: 8px; min-width: 130px; display: flex; flex-direction: column; box-shadow: 0 6px 16px rgba(0,0,0,0.5); position: relative; overflow: hidden; }
...
export class ReverbPedalComponent {
